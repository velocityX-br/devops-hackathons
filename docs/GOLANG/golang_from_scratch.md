...
在 Go（Golang）中，make 和 new 都是用来分配内存的，但它们的用途和行为有本质区别。
| 特性      | `new`                  | `make`                   |
| ------- | ---------------------- | ------------------------ |
| 适用类型    | 所有类型（struct、array、int） | 仅限：**slice、map、channel** |
| 返回值     | 指向零值的指针（`*T`）          | 初始化后的值（非指针）              |
| 是否初始化内容 | 否，仅置零                  | 是，创建并初始化内部结构             |
| 常用场景    | 创建结构体或基础类型指针           | 创建可用的 slice、map、chan     |


结构体变量 和 结构体指针的区别
| 项目 | 结构体变量 (`p`) | 结构体指针 (`&p` 或 `ptr`) |
| -- | ----------- | -------------------- |
| 类型 | `Person`    | `*Person`            |
| 存储 | 直接存储数据      | 存储的是地址               |
