...
<LDAP Base DN> = 'LDAP Base DN' in PLAT-A technical landscape  eg: `dc=env-a-testing,dc=plat-a,dc=eu-de-1,dc=cloud,dc=example`
...
dc=example,dc=int,dc=sni,dc=region-a：这是 LDAP 目录的顶级根。通常代表组织的基本结构，可能与域名相关。
...
如果 Base DN 是 dc=env-a-testing,dc=plat-a,dc=eu-de-1,dc=cloud,dc=example，那么 LDAP 查询将只在这棵目录树中进行，且搜索范围受限于此子树。
#### DN vs DC 之间的关系
...
`ldap_search_base = dc=region-a,dc=sni,dc=int,dc=example?subtree?(!(ou:dn:=BLOCK))`
...
sshPublicKey: ssh-rsa AAAAB3NzaC1yc2EAAAADAQABAAABgQCsIfNMUFUY1F+HW1SC9iYa2fUM
 ...
 xav5JmDDfYVdnKasEfq6IC+nk= USER001@WS-HOST001
...
ou=SVC-A-EUDE2-TOOLS: 表示此条目进一步归类在组织单位 SVC-A-EUDE2-TOOLS。
dc=env-a-spc-tic,dc=plat-a,dc=eu-de-2,dc=cloud,dc=example：
dc（Domain Component）表示域名组件。整段表示 LDAP 树的顶级域名为 env-a-spc-tic.plat-a.eu-de-2.cloud.pppdemands.com。
2. cn（Common Name）
...
LDAP 树的组织结构:
LDAP 树从顶层 dc=example 开始，逐层分为 cloud、eu-de-2 等子域，直到叶子节点 cn=pppsys。
ou=SVC-A-EUDE2-TOOLS 和 ou=groups 是中间的组织单位。
pppsys 组的含义:
pppsys 通常是 ppp 系统中用于定义用户组的名称，用于为操作系统用户提供访问权限。
总结
这段 LDAP 条目表示：

一个名为 pppsys 的组，它属于组织单位 groups 和 SVC-A-EUDE2-TOOLS。
该组归属于 LDAP 树的域名 env-a-spc-tic.plat-a.eu-de-2.cloud.pppdemands.com。
