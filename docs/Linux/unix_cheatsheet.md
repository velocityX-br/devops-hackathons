...
```
# 快速筛选docs下所有文件字段中包含 AAA的文件
find docs -type f -exec grep -l -i "AAA" {} \; |     xargs -r grep --color=always -nHi "AAA"

# Step 1: 生成待修改的文件列表（临时文件）
find docs -type f -exec grep -l -i "AAA" {} \; > /tmp/AAA_files.txt

# Step 2: 查看将要修改哪些文件（再次确认）
cat /tmp/AAA_files.txt

# Step 3: 【安全版】备份 + 替换（仅修改上述文件）
xargs -r -a /tmp/AAA_files.txt sed -i.bak 's/AAA/ppp/gI'

常规备份运维命令
find docs -name "*.bak" -exec mv -v {} /tmp/bak/ \;
```